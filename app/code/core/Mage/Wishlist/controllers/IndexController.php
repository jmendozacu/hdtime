...
class Mage_Wishlist_IndexController extends Mage_Wishlist_Controller_Abstract
{
    ...
    public function updateItemOptionsAction()
    {
        ...
        try {
            ...
            $message = $this->__('<span class="whishlist">%1$s has been updated in your wishlist.</span>', $product->getName());
            $session->addSuccess($message);
        } catch (Mage_Core_Exception $e) {
    ...
    public function fromcartAction()
    {
        ...
        try {
            ...
            $session->addSuccess(
                Mage::helper('wishlist')->__("<span class='whishlist'>%s has been moved to wishlist %s</span>", $productName, $wishlistName)
            );
            $wishlist->save();
        } catch (Mage_Core_Exception $e) {
            $session->addError($e->getMessage());
