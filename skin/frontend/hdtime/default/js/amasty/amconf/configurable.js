...
Product.Config.prototype.loadAdditionalContent = function(url, element) {
    ...
    if (confData.hasKey(key))
    {
        ...
        if (confData.getData(key, 'media_url'))
        {
            ...
            new Ajax.Updater(tmpContainer, confData.getData(key, 'media_url'), {
                ...
                {	
					var price = '';
					if (jQuery('#product_addtocart_form .special-price').size() > 0) {
						price += jQuery('#product_addtocart_form .special-price .price').html();
					} else if (jQuery('#product_addtocart_form .regular-price')) {
						price += jQuery('#product_addtocart_form .price-box .price').html();
					}
					
					jQuery('#product_shot_but .quick_body .title_quick_product b').html(price);
                },
                onCreate: function()
                {
        ...
        if (confData.getData(key, 'additional_content_url')) {
			
			var idProduct = parseInt(confData.getData(key, 'additional_content_url').replace(/\D+/g,""));
			jQuery('#id_product').val(idProduct);

            this.loadAdditionalContent(confData.getData(key, 'additional_content_url'), 'product_tabs_additional_contents')
        }
    } else 
    {
...
Product.Config.prototype.reloadSimplePrice = function(key)
{
    ...
    if (confData.hasKey(key))
    {
        ...
        if (confData.getData(key, 'price_html'))
        {
            ...
            $$('.product-options .price-box').each(function(container)
            {
