...
var CompareNoReload = {
    ...
    compareAddToList: function (addUrl,id2Animate)
    {
        ...
        new Ajax.Request(addUrl, {
				...
				var succRez = JSON.parse(response.responseText);
				var count = jQuery(succRez).find('.block-title small').html();
				count = count.replace(/[^-0-9]/gim,'');
				if (count == '') {
					count = 1;
				}
				//jQuery('.basket.top-aside ul.list-unstyled .first .link-kol').html(count);
				jQuery('.compare_count').html(count);
				...
				var name = localStorage.getItem('nameCompare');
				
				if (name != '') {
					var html = '';
					html += count;
					html += '<span class="compare-notice">';
					html += '<span class="compare-notice-body ">';
					html += '<span class="compare-notice-head">'+name+'</span>';
					html += 'добавлен в список сравнения</span>';
					html += '</span>';
					
					jQuery('.basket.top-aside ul.list-unstyled .first .link-kol').html(html);
					
					setTimeout(function(){
						jQuery('.compare-notice').fadeOut('fast');
						localStorage.setItem('nameCompare', '');
					}, 2000);
				}
            }
        });
    ...
    rescanForNewCompare: function()
    {
        ...
        for (var i=0;i<allOldLinks.length;i++) {
           if(allOldLinks[i].readAttribute("oldlink")==null)
			...
			var ee = document.getElementById("product_addtocart_form");//jQuery('#');
			//console.log(ee);
            if (fparent===null) {
                fparent=this.findParentByCss(allOldLinks[i],"li");
            }
			if (fparent===null) {
                fparent=document.getElementById("product_addtocart_form");
            }
